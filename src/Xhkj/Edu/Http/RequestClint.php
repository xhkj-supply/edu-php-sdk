...
class RequestClint
{
	...
	public static function get($methodOrUri,Base $Request)
	{
		...
		$headers = array(
			'access_token'=> $Request->paramMap['access_token'],
		);
	...
	public static function gettoken($methodOrUri,Base $Request)
	{
		...
		$Request->addParam('sign', self::setSign($Request->getAllParam(), $Request->getAppSecret()));
		$serverUrl = self::richRequest($methodOrUri, $Request);
		...
		return self::curl_request($serverUrl, json_encode($Request->paramMap),$headers);
	}
